...
// handle Page 2: you can access new page DOM through newPage object
		...
		it('It should fill out the employment tab', async () => {
			...
			await newPage.keyboard.press('Tab')
			await delay(2000)
			await newPage.keyboard.press('1')
		...
		it('should close the browser', async() => {
            //const dashboardSideLink = await newPage.$x("//span[text()='Dashboard']")
			//await dashboardSideLink[0].click()
            await delay(6000)
            await newPage.click('span.icon.-nav-dashboard')
            await delay(6000)
            await browser.close()	
		})
